...
A framework-neutral TypeScript core for a portfolio assistant. It keeps all site-specific material in one validated config object, so the chat can be reused across personal sites without copying server code.
...
Use [`src/config/example.ts`](src/config/example.ts) as the starting config. Keep profile text factual, current, and free of secrets; it becomes the model’s source of truth.
...
The core deliberately takes a `LangfusePort` rather than importing a particular Langfuse SDK version. In your server integration, construct the SDK client only when `LANGFUSE_PUBLIC_KEY` and `LANGFUSE_SECRET_KEY` exist, then pass an adapter with `trace` and optionally `getPrompt`. If the client, network, or remote prompt fails, chat continues with the local portfolio prompt.

`getPrompt` should return an object whose `compile({ name, headline, question, context })` method returns a string. This makes Langfuse prompt management opt-in while keeping a reviewed local fallback in source control.
